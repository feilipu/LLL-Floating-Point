...
INT0_FPU        .EQU     $3900 ; start of the FPU Interrupt 1 asm code (RAM)
...
;
        ...
        .ORG INT0_FPU           ;LIBRARY ORIGIN FOR RC2014 AND YAZ180 DURING TESTING
...
APU_ISR:
        push af                 ; store A so we don't clobber it
        push bc                 ; store BC so we don't clobber it        
...
APU_ISR_ENTRY:
        ...
        jp z, APU_ISR_OP_ENT    ; load an OPERAND
        
        cp OP_REM_CMD           ; check whether it is OPERAND removal
        jp z, APU_ISR_OP_REM    ; remove an OPERAND
...
;        jr nc, APU_OP_EXIT      ; COMMAND buffer full, so pop the data and exit
...
;        pop af                  ; pop the operand entry COMMAND        
;        jr nc, APU_CMD_EXIT     ; COMMAND buffer full, so pop the data and exit
